...
interface Thread {
  jobId: string;
  jobTitle: string;
  otherParty: { name: string; photo?: string | null };
  myRole: 'artisan' | 'customer';
...
export default function ArtisanMessagesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setThreads(convos.map((j: {
        ...
        title?: string;
        category?: string;
        customerId?: { _id: string; name: string; profilePhoto?: string | null } | null;
        assignedArtisanId?: { _id: string; name: string; profilePhoto?: string | null } | null;
        lastMessage?: { text: string; at: string };
      }) => {
        // Determine which party the current user is in this job
        const iAmCustomer = j.customerId?._id?.toString() === user.id?.toString()
          || j.customerId?._id?.toString() === (user as { _id?: string })._id?.toString();
        const myRole: 'artisan' | 'customer' = iAmCustomer ? 'customer' : 'artisan';
        const otherParty = iAmCustomer
          ? { name: j.assignedArtisanId?.name ?? 'Artisan',  photo: j.assignedArtisanId?.profilePhoto ?? null }
          : { name: j.customerId?.name ?? 'Customer',        photo: j.customerId?.profilePhoto ?? null };
  ...
  return (
    <div className="py-8 px-4 md:px-8 max-w-2xl">
      ...
      <p className="text-[14px] text-on-surface-variant mb-6">All your job conversations</p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <p className="text-[14px] text-on-surface-variant">Conversations on your jobs will appear here.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {threads.map((t) => {
            const href = t.myRole === 'artisan'
              ? `/artisan/messages/${t.jobId}`
              : `/customer/messages/${t.jobId}`;
            return (
              <Link
                key={t.jobId}
                href={href}
              ...
              >
                ...
                <div className="relative flex-shrink-0">
                  <div className="w-14 h-14 rounded-xl overflow-hidden bg-primary-container"
                    ...
                    {t.otherParty.photo ? (
                      <Image src={t.otherParty.photo} alt={t.otherParty.name} width={56} height={56} className="object-cover w-full h-full" />
                    ) : (
                      <div className="w-full h-full bg-primary-container flex items-center justify-center">
                        <span className="text-[18px] font-black text-primary/50">{getInitials(t.otherParty.name)}</span>
                      </div>
                    )}
                  </div>
                  {t.unread && (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    {t.myRole === 'customer' && (
                      <span className="text-[10px] font-bold px-1.5 py-0 rounded-full bg-amber-100 text-amber-700 flex-shrink-0">
                        As Customer
                      </span>
                    )}
                  </div>
