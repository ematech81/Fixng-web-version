...
interface Thread {
  jobId: string;
  jobTitle: string;
  customer: { name: string; photo?: string | null };
...
export default function ArtisanMessagesPage() {
    ...
    Promise.all([
      ...
      setThreads(convos.map((j: {
        ...
        customer:  {
          ...
          photo: j.customerId?.profilePhoto ?? null,
        },
  ...
  return (
    <div className="py-8 px-4 md:px-8 max-w-2xl">
      ...
      <p className="text-[14px] text-on-surface-variant mb-6">Chat with your customers</p>
            ...
            >
              ...
              <div className="relative flex-shrink-0">
                <div className="w-14 h-14 rounded-xl overflow-hidden bg-primary-container"
                  ...
                  {t.customer.photo ? (
                    <Image src={t.customer.photo} alt={t.customer.name} width={56} height={56} className="object-cover w-full h-full" />
                  ) : (
                    <div className="w-full h-full bg-primary-container flex items-center justify-center">
                      <span className="text-[18px] font-black text-primary/50">{getInitials(t.customer.name)}</span>
                    </div>
                  )}
                </div>
                {t.unread && (
