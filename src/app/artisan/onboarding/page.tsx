...
type Step = 'photo' | 'skills' | 'location' | 'verification' | 'video' | 'review' | 'pending';
...
const STEP_PREV: Partial<Record<Step, Step>> = {
  skills: 'photo', location: 'skills', verification: 'location', video: 'verification', review: 'video',
};
const STEP_META: Record<Step, { title: string; subtitle: string; icon: string }> = {
  ...
  verification: { title: 'Verify Your Identity',  icon: 'verified_user', subtitle: 'Optional — upload a government-issued ID to earn your Verified badge. You can do this later.' },
  video:        { title: 'Skill Video',           icon: 'videocam',      subtitle: 'A short video of your work helps you stand out. Optional — you can skip.' },
  review:       { title: 'Almost Done!',          icon: 'task_alt',      subtitle: '' },
...
export default function ArtisanOnboardingPage() {
  ...
  const handleIdSubmit = async () => {
    // Nothing selected — auto-skip (step is optional)
    if (!idFile && !idIsExisting) { await handleSkipId(); return; }
    // Has existing upload and no replacement — just advance
    if (idIsExisting && !idFile) { advanceTo('video'); return; }
    // Uploading a new file — idType is required
  ...
  const handleVideoSubmit = async () => {
    ...
    if (videoIsExisting && !videoFile)  { advanceTo('review'); return; }
  ...
  const isLoading =
    ...
    step === 'video'         ? videoUploading :
    step === 'review'        ? registering : false;
  ...
  const handleContinue = () => {
    switch (step) {
      ...
      case 'review':       return fetchAndShowPending();
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#f8f9fb' }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-8">
        ...
        {stepNum
          ? <span className="text-[13px] font-semibold text-gray-500">Step {stepNum} of 5</span>
          : step === 'review'
            ? <span className="text-[13px] font-semibold text-gray-500">Final Step</span>
            : null
        }
        {step !== 'pending'
          ? (
      ...
      {(stepNum || step === 'review') && (
        <div className="fixed top-16 left-0 right-0 z-40 h-1 bg-gray-100">
          <div
            ...
            style={{ width: step === 'review' ? '100%' : `${(stepNum! / 5) * 100}%`, background: '#004ac6' }}
          />
        </div>
      )}
      ...
      <main className="flex-1 pt-20 pb-28 px-4">
        {initLoading ? (
          ...
          <div className={`mx-auto ${step === 'skills' ? 'max-w-3xl' : 'max-w-xl'}`}>
            ...
            {/* Step header (hidden on pending and review — both have their own UI) */}
            {step !== 'pending' && step !== 'review' && (
            ...
            {step === 'verification' && (
              <div className="space-y-5">
                ...
                {/* ID type selector */}
                <div>
                  <label className="block text-[13px] font-semibold text-gray-600 mb-2">Document Type <span className="text-gray-400 font-normal">(required if uploading)</span></label>
                  ...
                  >
                    {idFile ? (
                      ...
                      <>
                        ...
                        <p className="text-[11px] mt-2" style={{ color: '#004ac6' }}>Leave empty and tap Continue to skip</p>
                      </>
                    )}
                  </div>
            ...
            {step === 'review' && (
              <div className="max-w-lg mx-auto space-y-6">
                ...
                <div className="text-center">
                  <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: '#e6f4ed' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '44px', color: '#006229', fontVariationSettings: "'FILL' 1" }}>task_alt</span>
                  </div>
                  <h1 className="text-[28px] font-black text-gray-900 mb-2">Almost Done!</h1>
                  <p className="text-[15px] text-gray-500 leading-relaxed">
                    Review your profile and click <strong className="text-gray-700">Complete Registration</strong> to go live and start receiving job requests.
                  </p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  {[
                    { label: 'Profile Photo',   done: !!photoPreview,                     optional: false },
                    { label: 'Skills & Bio',    done: selectedSkills.length > 0,           optional: false },
                    { label: 'Location',        done: !!stateVal,                          optional: false },
                    { label: 'ID Verification', done: idIsExisting || !!idFile,            optional: true  },
                    { label: 'Skill Video',     done: videoIsExisting || !!videoFile,      optional: true  },
                  ].map(({ label, done, optional }, i, arr) => (
                    <div
                      key={label}
                      className={`flex items-center gap-3 px-5 py-4 ${i < arr.length - 1 ? 'border-b border-gray-100' : ''}`}
                    >
                      <div
                        className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                        style={{ background: done ? '#e6f4ed' : '#f5f5f5' }}
                      >
                        <span
                          className="material-symbols-outlined"
                          style={{ fontSize: '18px', color: done ? '#006229' : '#bdbdbd', fontVariationSettings: "'FILL' 1" }}
                        >
                          {done ? 'check_circle' : 'radio_button_unchecked'}
                        </span>
                      </div>
                      <span className="flex-1 text-[15px] font-semibold text-gray-700">{label}</span>
                      {optional && !done && (
                        <span className="text-[12px] text-gray-400 font-medium bg-gray-100 px-2 py-0.5 rounded-full">Skipped</span>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <p className="text-[13px] text-gray-400 text-center">
                  You can add your ID and skill video later from your profile settings to unlock your Verified badge.
                </p>
              </div>
            )}
      ...
      {step !== 'pending' && !initLoading && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 md:px-8 py-4">
          <div className={`mx-auto flex items-center gap-3 ${step === 'skills' ? 'max-w-3xl' : 'max-w-xl'}`}>
            ...
            {/* Skip (video only — verification auto-skips via Continue) */}
            {step === 'video' && (
              <button
      ...
      {showCongrats && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.65)' }}>
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            {/* Tick icon */}
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5" style={{ background: '#e6f4ed' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '44px', color: '#006229', fontVariationSettings: "'FILL' 1" }}>check_circle</span>
            </div>
            <h2 className="text-[24px] font-black text-gray-900 mb-2 leading-tight">
              Registration Successful!
            </h2>
            <p className="text-[15px] text-gray-500 mb-1 leading-relaxed">
              Welcome to FixNG! Your artisan profile is now live and ready to receive job requests from customers.
            </p>
            <p className="text-[13px] font-semibold mb-6" style={{ color: '#006229' }}>
              Click the link below to access your artisan dashboard.
            </p>
            <button
              onClick={() => router.replace('/artisan/dashboard')}
              className="w-full py-4 rounded-2xl font-bold text-[16px] text-white transition-all hover:brightness-110 flex items-center justify-center gap-2"
              style={{ background: '#004ac6' }}
            >
              Access your Artisan Dashboard
              <span className="material-symbols-outlined" style={{ fontSize: '20px', fontVariationSettings: "'FILL' 1" }}>arrow_forward</span>
            </button>
          </div>
        </div>
      )}
